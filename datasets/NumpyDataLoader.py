...
class NumpyDataSet(object):
    ...
    def __init__(self, base_dir, mode="train", batch_size=16, num_batches=10000000, seed=None, num_processes=8, num_cached_per_queue=8 * 4, target_size=128,
                 file_pattern='*.npy', label_slice=1, input_slice=(0,), do_reshuffle=True, keys=None, additional_slices=2):
        ...
        data_loader = NumpyDataLoader(base_dir=base_dir, mode=mode, batch_size=batch_size, num_batches=num_batches, seed=seed, file_pattern=file_pattern,
                                      input_slice=input_slice, label_slice=label_slice, keys=keys, additional_slices=additional_slices)

        self.data_loader = data_loader
        self.batch_size = batch_size
        self.do_reshuffle = do_reshuffle
        self.number_of_slices = 2*additional_slices + 1

        self.transforms = get_transforms(mode=mode, target_size=target_size, reshape_factor=1, number_of_slices=self.number_of_slices)
...
class NumpyDataLoader(SlimDataLoaderBase):
    def __init__(self, base_dir, mode="train", batch_size=16, num_batches=10000000,
                 seed=None, file_pattern='*.npy', label_slice=1, input_slice=(0,), keys=None, additional_slices=2):

        self.additional_slices = additional_slices

        self.files, self.file_len, self.slices = load_dataset(base_dir=base_dir, pattern=file_pattern, slice_offset=additional_slices, keys=keys, )
    ...
    def get_data_from_array(self, open_array):
        ...
        for slice in open_array:
            ...
            numpy_slice = numpy_array[:, slice[1] - self.additional_slices:slice[1] + 1 + self.additional_slices, ]
            data.append(numpy_slice[self.input_slice[0]])  # 'None' keeps the dimension

            if self.label_slice is not None:
                labels.append(numpy_slice[self.label_slice[0]])
