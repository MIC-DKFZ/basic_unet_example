...
class NumpyDataSet(object):
    ...
    TODO
    """
    def __init__(self, base_dir, mode="train", batch_size=16, num_batches=100, seed=None, num_processes=8, num_cached_per_queue=1, target_size=128,
                 file_pattern='*.npy', label_slice=3, input_slice=(0,1,2), do_reshuffle=False, keys=None):
...
class NumpyDataLoader(SlimDataLoaderBase):
    def __init__(self, base_dir, mode="train", batch_size=16, num_batches=100,
                 seed=None, file_pattern='*.npy', label_slice=3, input_slice=(0,1,2), keys=None):
    ...
    def get_data_from_array(self, open_array):
        ...
        for slice in open_array:
            ...
            if self.label_slice is not None:
                labels.append(numpy_slice[list(self.label_slice)])  # 'None' adds 1 dimension
