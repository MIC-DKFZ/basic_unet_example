...
class UNetExperiment(PytorchExperiment):
    ...
    def setup(self):
        ...
        self.train_data_loader = NumpyDataSet(self.config.data_dir, target_size=self.config.patch_size, batch_size=self.config.batch_size,
                                              keys=tr_keys, label_slice=None,  input_slice=(0, 1, 2, 3, ))
        ...
        self.val_data_loader = NumpyDataSet(self.config.data_dir, target_size=self.config.patch_size, batch_size=self.config.batch_size,
                                            keys=val_keys, mode="val", do_reshuffle=False, label_slice=None,  input_slice=(0, 1, 2, 3, ))
        self.test_data_loader = NumpyDataSet(self.config.data_test_dir, target_size=self.config.patch_size, batch_size=self.config.batch_size,
                                             keys=test_keys, mode="test", do_reshuffle=False, label_slice=None,  input_slice=(0, 1, 2, 3,))
        self.model = UNet(num_classes=self.config.num_classes, in_channels=self.config.in_channels, num_downs=4)
        ...
        #self.vlog.plot_model_structure(self.model,
         #                               [self.config.batch_size, 1, self.config.patch_size, self.config.patch_size],
          #                              name='Model Structure') #name is 'experiment'
    ...
    def train(self, epoch):
        ...
        for data_batch in self.train_data_loader:
            ...
            if (batch_counter % self.config.plot_freq) == 0:
                ...
                self.clog.show_image_grid(target.float().cpu(), name="mask_train", title="Mask", n_iter=epoch)
    ...
    def test(self):
        ...
        with torch.no_grad():
            for data_batch in self.test_data_loader:
                ...
                # Get data_batches
                mr_data = data_batch['data'][0, :, 0:3, :, :].float().to(self.device)
                mr_target = data_batch['data'][0, :, 3:4, :, :].float().to(self.device)
